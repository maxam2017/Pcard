...
function pack(content) {
    return content.replace(/http\S*jpg|http\S*jpeg|http\S*png|http\S*gif/g, "<img src='$&' width='80%' style='margin:5% auto'>").replace(/\n/g, "<br>");
}

function dat(date) {
    //example 2017-11-06T16:45:30.948Z
    var now = new Date();
...
function paste(e) {
    ...
    $(".card").click(function detail() {
        ...
        $.ajax({
            ...
            error: function () {
                if ($(window).width() > 768) {
                    $("#container").append("<div id='l'></div><div class='post' style='background-color:#333;color:#eee;text-align:center;font-size:1.4em;overflow:auto;'><img src='icon/error.svg' width='50%' style='display:block;margin:5% auto;'>Oh！這篇文章已被作者刪除或被抓走了...</div><div id='r'></div>");
                    /*********************post set********************************/
                    css_post();
                    ...
                    card.append("<div class='err' style='background-color:#333;color:#eee;text-align:center;font-size:1.4em;overflow:auto;'><img src='icon/error.svg' width='50%' style='display:block;margin:5% auto;'>Oh！這篇文章已被作者刪除或被抓走了...</div>");
            ...
            success: function (e) {
                var icon;
                if (e.gender == "F") icon = "icon/female.svg";
                else if (e.gender) icon = "icon/male.svg";
                else icon = "icon/angel.svg";
                ...
                if ($(window).width() > 768) {
                    css_post();
                    $("#container").append("<div id='l'></div><div class='post'><div class='post_title'>" + "<img src='" + icon + "' height=100% style='margin-right:10px;' >" + e.title + "<span style='position:absolute;right:10px;top:5%;font-size:14px;color:#7833a5;'>" + date + "</span></div>" + "<div class='article'>" + content + "</div><div class='bar'><img src='icon/like.svg' style ='margin: 0 10px;'>" + "<span style='color:#bd494a;'>" + like + "</span>" + "<img src='icon/comment.svg' style ='margin: 0 10px;'>" + "<span style='color:#006aa6;'>" + comment + "</span></div></div><div id='r'></div>");
                    /*********************post set********************************/
                    $(".post").css("top", $(window).scrollTop());
                    $("#l").css("top", $(window).scrollTop());
                    $("#r").css("top", $(window).scrollTop());
                    ...
                    card.contents(".content").hide();
                    card.append("<div class='article'>" + content + "</div><div class='bar'><img src='icon/like.svg' style ='margin: 0 10px;'>" + "<span style='color:#bd494a;'>" + like + "</span>" + "<img src='icon/comment.svg' style ='margin: 0 10px;'>" + "<span style='color:#006aa6;'>" + comment + '</span>');
                    card.off("click", detail);
                    card.click(function excerpt() {
                        card.contents(".content").show();
                ...
                $.ajax({
                    ...
                    success: function (e) {
                        for (i = 0; i < e.length; i++) {
                            ...
                            //icon set
                            var icon;
                            if (e[i].gender == "F") icon = "icon/female.svg";
                            else if (e[i].gender) icon = "icon/male.svg";
                            else icon = "icon/angel.svg";
                            var content = pack(e[i].content);
                            //show
                            if ($(window).width() > 768) {
                                $(".post").append("<div class='comment'><div class='comment_title'><img src='" + icon + "' height=80% style='margin-right:5px; vertical-align:middle;' >" + sch + "<span style='position:absolute;right:10px;color:#7833a5;font-size:14px;'>B" + e[i].floor + "</span>" + "</div>" + "<div class='content'>" + content + "</div>");
                            } else {
                                card.append("<div class='comment'><div class='comment_title'><img src='" + icon + "' height=80% style='margin-right:5px ;' >" + sch + "<span style='position:absolute;right:10px; color:#7833a5;font-size:10px;'>B" + e[i].floor + "</span>" + "</div>" + "<div class='content'>" + content + "</div>");
                            }
                        }
                    }
                });
    ...
    $.ajax({
        ...
        success: function (e) {
            ...
            var like = e.likeCount,
                comment = e.commentCount;
            var content = pack(e[i].content);
            $("#container").append("<div id='l'></div><div class='post' id='" + e.id + "'><div class='title'>" + "<img src='" + icon + "' height=100% style='margin-right:10px;' >" + e.title + "</div>" + "<div class='article'>" + content + "</div><div class='bar'><img src='icon/like.svg' style ='margin: 0 10px;'>" + "<span style='color:#bd494a;'>" + like + "</span>" + "<img src='icon/comment.svg' style ='margin: 0 10px;'>" + "<span style='color:#006aa6;'>" + comment + '</span>' + "</div></div><div id='r'></div>");
            /*********************post set********************************/
            getComment(url);
            css_post();
            $(".post").css("top", $(window).scrollTop());
    ...
    $.ajax({
        ...
        success: function (e) {
            for (i = 0; i < e.length; i++) {
                ...
                //show
                $(".post").append("<div class='comment'><div class='figure'><img src='" + icon + "' height=80% style='margin-right:5px;' >" + sch + "<br></div>" + "<div class='content'>" + content + "</div>");
            }
        }
    });
}
...
$(function () {
    ...
    $(window).resize(function () {
        ...
        if ($(window).width() < 350) $("#search").hide();
        else $("#search").show();
    });
});
/*
